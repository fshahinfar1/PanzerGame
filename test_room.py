...
class TestRoom(room_obj.Room):
    ...
    def process_events(self):
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                self.Players[0].get_controller().get_event(event)
    ...
    def run_logic(self):
        # player1
        # turn
        if self.Players[0].get_controller().is_key_hold('right'):
            self.Players[0].get_panzer().key_right()
        elif self.Players[0].get_controller().is_key_hold('left'):
            self.Players[0].get_panzer().key_left()
        # forward / backward
        if self.Players[0].get_controller().is_key_hold('up'):
            self.Players[0].get_panzer().key_up()
        elif self.Players[0].get_controller().is_key_hold('down'):
            self.Players[0].get_panzer().key_down()
        else:
            self.Players[0].get_panzer().set_acceleration(0)
        # fire
        if self.Players[0].get_controller().is_key_hold('space'):
            load = self.Players[0].get_panzer().key_space()
            if load is not None:
